...
import "./App.css";
import { userInfo, secctionCopy, modalssss } from "../setting.js";
...
function App() {
  ...
  // Hàm lấy ngày giờ hiện tại ở Việt Nam
  const getVietnamDateTime = () => {
    const now = new Date();
    const vietnamDateTime = format(now, "HH:mm:ss - dd.MM.yyyy", {
      timeZone: "Asia/Ho_Chi_Minh",
    });
    return vietnamDateTime;
  };

  // Hàm xử lý khi click vào dòng ngày giờ
  ...
  <div class="flex items-center space-x-4">
    ...
    <div class="text-balance">
      ...
      <a
        href="https://www.facebook.com/adelaineuygn" // Replace with actual link
        target="_blank"
        rel="noopener noreferrer"
        className="hover:underline"
      >
        <b>Nguyen Phuong Lien</b>
      </a>
    </div>
  </div>
</section>
        ...
        <footer className="space-y-4 pb-8 pt-4">
          <div className="flex flex-col items-center space-y-3">
            <div className="flex items-center space-x-2 text-sm text-slate-400">
              <span onClick={toggleDateTime}>
                {!showDateTime && "Vietnam - "}
                {showDateTime ? (
                  <>🇻🇳 Ho Chi Minh city, Vietnam - {getVietnamDateTime()}</>
                ) : (
                  currentTime
                )}
              </span>
